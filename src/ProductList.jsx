...
function ProductList({ onHomeClick }) {
    ...
    const cartItems = useSelector(state => state.cart.items); // Read cart items from the Redux store
    const totalCartQuantity = cartItems.reduce((total, item) => total + item.quantity, 0); // Total number of items in the cart
                            ...
                            <div className="product-list"> {/* Container for the list of plant cards */}
                                {category.plants.map((plant, plantIndex) => ( // Loop through each plant in the current category
                                    <div className="product-card" key={plantIndex}> {/* Unique key for each plant card */}
                                        ...
                                        <button
                                            className={`product-button ${addedToCart[plant.name] ? 'added-to-cart' : ''}`} // Grey out the button once added
                                            onClick={() => handleAddToCart(plant)} // Handle adding plant to cart
                                            disabled={addedToCart[plant.name]} // Disable the button after the plant is added
                                        >
